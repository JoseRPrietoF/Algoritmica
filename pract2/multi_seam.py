...
def remove_seams(height,seam_paths, matrix):
    ...
    # print("vamos a borrar %d " % len(seam_paths))
    #ahora guardaremos aqui los pixels a borrar pero por filas
    new_paths = []
    for p in range(len(seam_paths[0])):
        new_paths.append([])
    #giramos
    ...
    for i in range(len(new_paths)):
        ...
        for j in path_row:
            ...
            matrix[i].pop(j)
            # except IndexError:
            #     print("Longitud de matrix[%d] %d"%(i,len(matrix[i])))
            #     print("i : %d" % i)
            #     print("j (pop) :% d" % j)
            #     print(path_row)
            #     print(seam_paths)
...
def dp_seam_carving_multi(grad,mat,N,pscore=0.8):
    ...
    infty = 1e99
    """ aqui """
    #he tenido que anyadir esta linea porque no variaba la anchura de mat..
    mat = [[infty for x in range(width)] for y in range(height)]
    ...
    # print(len(mat[height-1])) #no cambia la anchura de mat!
    # print(width)
    ...
    score = bscore
    # print("medidas %d %d " % (len(grad), len(grad[0])))
    # print(min_scores)
    #condicion de parada
    ...
    #Ahora escogemos solo los buenos
    usados = dict()
    paths_validos = []
    for path in paths:
